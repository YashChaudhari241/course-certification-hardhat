...
contract Certify {
    ...
    modifier onlyAuthorities {
        require(authorities[msg.sender] ==1 || msg.sender == i_owner, "Not an Authority");
        _;
    }
    modifier onlyOwner {
    ...
    modifier onlyIssuer {
        require(issuers[msg.sender] ==1, "Only issuers can call this function");
        _;
    }
    ...
    function getAllCertificates(address user) external view returns (Certificate[] memory){
        return certificates[user];
    }
    function getCertificate(address user,uint index) external view returns (Certificate memory){
        if(certificates[user].length <= index){
            revert Certify__OutOfIndex();
        }
